...
export async function POST(request: Request) {
  ...
  if (!validTypes.includes(type as RequestType)) {
    return NextResponse.json(
      { error: "Tipo de requisição inválido. Deve ser um dos seguintes: REQUISITION, RETURN, SUBSTITUTION." },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    // Translate request type to Mozambican Portuguese
    ...
    const translatedType = typeTranslations[type as RequestType] || type;
      ...
      const notificationPromises = Array.from(usersToNotify.values()).map((user) => {
        ...
        return NotificationService.createNotification({
          ...
          data: {
            ...
            requesterName: requester_name,
            message: isApprover
              ? `Uma nova requisição de ${translatedType.toLowerCase()} (${request_number}) requer a sua aprovação.`
              : `Uma nova requisição de ${translatedType.toLowerCase()} (${request_number}) foi criada para o seu grupo (${user.groupName}).`,
          },
        });
      });
    ...
    return NextResponse.json({ error: "Falha ao criar a requisição" }, { status: 500 });
  }
}