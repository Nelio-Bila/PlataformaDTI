    ...
    <div className="p-6 space-y-6">
      ...
      <Tabs defaultValue="overview" value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        <TabsList>
          ...
          {canViewEquipment && <TabsTrigger value="equipment">Equipamentos</TabsTrigger>}
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            {canViewEquipment && (
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">{equipment?.total_equipment || 0}</div>
                  <p className="text-xs text-muted-foreground">
                    {equipment?.equipment_by_type?.length || 0} diferentes tipos
                  </p>
                </CardContent>
              </Card>
            )}
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{requests.total_my_requests}</div>
                <p className="text-xs text-muted-foreground">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {requests.request_by_status.find(s => s.status === RequestStatus.PENDING)?.count || 0}
                </div>
                <p className="text-xs text-muted-foreground">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">{requests.department_requests.length}</div>
                <p className="text-xs text-muted-foreground">
              ...
              <CardContent>
                <div className="space-y-4">
                  {requests.recent_requests.length > 0 ? (
                    ...
                    <p className="text-sm text-muted-foreground">Nenhuma solicitação recente encontrada.</p>
                  )}
                </div>
              </CardContent>
            </Card>
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      {equipment?.recent_equipment?.map((equip) => (
                        <tr key={equip.id} className="border-b hover:bg-muted/50">
                          <td className="p-2">{equip.serial_number || 'N/A'}</td>
                          <td className="p-2">{equip.type}</td>
                          <td className="p-2">{equip.brand} {equip.model}</td>
                          <td className="p-2">{equip.department?.name || 'N/A'}</td>
                          <td className="p-2">
                            <Badge
                              style={{ backgroundColor: statusColors[equip.status as keyof typeof statusColors] }}
                            >
                              {equip.status}
                            </Badge>
                          </td>
                          <td className="p-2">{formatDistanceToNow(new Date(equip.created_at), { addSuffix: true })}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
        ...
        <TabsContent value="requests" className="space-y-4">
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={requests.request_by_status}>
                    ...
                    <YAxis />
                    <Tooltip formatter={(value, name, props) => [value, formatStatus(props.payload.status)]} />
                    <Bar dataKey="count" fill="#00C49F">
                      {requests.request_by_status.map((entry, index) => (
                        <Cell
                          key={`cell-${index}`}
                          fill={statusColors[entry.status as keyof typeof statusColors] || "#00C49F"}
                        />
                      ))}
                    </Bar>
                    </BarChart>
                ...
                <ResponsiveContainer width="100%" height={300}>
                  <PieChart>
                    <Pie
                      ...
                      outerRadius={80}
                      fill="#8884d8"
                      label={({ name, percent }) => `${formatRequestType(name)} ${(percent * 100).toFixed(0)}%`}
                    >
                      {requests.request_by_type.map((entry, index) => (
                        <Cell
                          key={`cell-${index}`}
                          fill={REQUEST_COLORS[entry.type as keyof typeof REQUEST_COLORS] || PIE_COLORS[index % PIE_COLORS.length]}
                        />
                      ))}
                    </Pie>
                    <Tooltip formatter={(value, name) => [value, formatRequestType(name as string)]} />
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Distribuição de requisições ao longo do ano</CardDescription>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={350}>
                <BarChart data={requests.monthly_requests}>
                  ...
                  <Tooltip />
                  <Legend />
                  <Bar dataKey="requisitions" name="Requisições" stackId="a" fill={REQUEST_COLORS.REQUISITION} />
                  <Bar dataKey="returns" name="Devoluções" stackId="a" fill={REQUEST_COLORS.RETURN} />
                  <Bar dataKey="substitutions" name="Substituições" stackId="a" fill={REQUEST_COLORS.SUBSTITUTION} />
                </BarChart>
              </ResponsiveContainer>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              <CardTitle>Requisições Recentes</CardTitle>
              <CardDescription>Detalhes das últimas requisições registradas</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="overflow-x-auto">
                <table className="w-full">
                  ...
                  <tbody>
                    {requests.recent_requests.map((request) => (
                      <tr key={request.id} className="border-b hover:bg-muted/50">
                        ...
                        <td className="p-2">{formatRequestType(request.type)}</td>
                        <td className="p-2">{request.requester?.name || 'N/A'}</td>
                        <td className="p-2">{request.requester_department?.name || 'N/A'}</td>
                        <td className="p-2">{request.destination_department?.name || 'N/A'}</td>
                        <td className="p-2">
                          <Badge
                            style={{ backgroundColor: statusColors[request.status as keyof typeof statusColors] }}
                          >
                        ...
                        <td className="p-2">{formatDistanceToNow(new Date(request.created_at), { addSuffix: true })}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
