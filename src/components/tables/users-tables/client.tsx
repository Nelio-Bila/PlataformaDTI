...
//       const response = await fetch(`/api/users/groups/${groupId}/users`, {
//         method: "POST",
//         headers: { "Content-Type": "application/json" },
...
export function UsersClient() {
  ...
  const availableGroups = groupsData?.filter(
    (group) => !selectedUser?.groups.some((g) => g.group.id === group.id)
  ) || [];
          ...
          <div className="space-y-4">
            ...
            <div className="relative">
              <Button
                ...
                disabled={isGroupsLoading}
              >
                {comboboxSearch || "Adicionar grupo"}
                <span>▼</span>
              </Button>
              {comboboxOpen && (
                <div className="absolute z-10 bg-background w-full mt-1  border rounded-md shadow-lg">
                  <Command>
                    <CommandInput
                      placeholder="Pesquisar grupos..."
                      value={comboboxSearch}
                      onValueChange={setComboboxSearch}
                      className="w-full p-2 border-b bg-background"
                    />
                      ...
                      <CommandGroup>
                        {availableGroups.map((group) => (
                          <CommandItem
                            key={group.id}
                            onSelect={() => {
                              addUserToGroup({ groupId: group.id, userId: selectedUserId! });
                            }}
                            className="p-2 hover:bg-gray-100 dark:hover:bg-blue-900 cursor-pointer"
                          >
                            {group.name}
                          </CommandItem>
                        ))}
