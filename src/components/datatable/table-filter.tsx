...
// src/components/datatable/table-filter.tsx
import { Button } from "@/components/ui/button";
import {
  Command,
  CommandEmpty,
...
interface Params {
  filters: string[]; // Change to required
}
...
const TableFilter: React.FC<TableFilterProps> = ({
  ...
  const onSelectFilter = (value: string) => {
    const newFilter = `${filter}:${value}`;
    const filters = params.filters.includes(newFilter)
      ? params.filters.filter((f) => f !== newFilter)
  ...
  const clearFilters = () => {
    setSelectedValues([]);
    setParams({ filters: params.filters.filter((f) => !f.startsWith(`${filter}:`)) });
  };
    ...
    <Popover>
      <PopoverTrigger asChild>
        <Button
          variant="outline"
          size="sm"
          className="w-full md:w-auto h-8 border-dashed border-gray-400 text-xs hover:bg-gray-200/50 dark:text-white"
        >
          <CirclePlusIcon className="mr-2 h-4 w-4" />
          {title}
          {selectedValues.length > 0 && <FilterBadges selectedValues={selectedValues} options={options} />}
