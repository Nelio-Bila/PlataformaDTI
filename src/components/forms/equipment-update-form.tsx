...
export function EquipmentUpdateForm({ equipmentId }: EquipmentUpdateFormProps) {
  ...
  const form = useForm<EquipmentFormData>({
    ...
    defaultValues: {
      ...
      purchase_date: "",
      warranty_end: "",
      status: "ACTIVO", // Default to a valid enum value
      direction_id: "",
      department_id: "",
      sector_id: "",
      service_id: "",
  ...
  useEffect(() => {
    if (equipmentData?.equipment && !isEquipmentLoading) {
      ...
      const extraFields = equipment.extra_fields || {};
      const parsedKeyValuePairs = Object.entries(extraFields).map(([key, value]) => ({
        key,
        value: String(value),
      }));
  ...
  const { data: departments = [], isLoading: isDepartmentsLoading } = useQuery({
    ...
    queryFn: () =>
      fetch(`/api/equipment/departments/${selectedDirectionId}`).then((res) => res.json()),
  ...
  const { data: repartitions = [], isLoading: isRepartitionsLoading } = useQuery({
    ...
    queryFn: () =>
      fetch(`/api/equipment/repartitions/${selectedDepartmentId}`).then((res) => res.json()),
    enabled: !!selectedDepartmentId,
  });
  ...
  const { data: services = [], isLoading: isServicesLoading } = useQuery({
    ...
    queryFn: () =>
      fetch(
        `/api/equipment/services?directionId=${selectedDirectionId || ""}&departmentId=${selectedDepartmentId || ""
        }`
      ).then((res) => res.json()),
  ...
  const on_submit = (values: EquipmentFormData) => {
    ...
    if (values.extra_fields && Object.keys(values.extra_fields).length > 0) {
      formData.append("extra_fields", JSON.stringify(values.extra_fields));
    }
                  ...
                  <div className="space-y-4">
                    <div className="flex gap-2">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          if (newKey.trim() && newValue.trim()) {
                            const newPair = { key: newKey.trim(), value: newValue.trim() };
                            ...
                            <li key={index} className="flex items-center gap-2">
                              <Input
                                ...
                                onChange={(e) => {
                                  ...
                                  const updatedPairs = [...keyValuePairs];
                                  updatedPairs[index] = { ...updatedPairs[index], key: newKey };
                                  setKeyValuePairs(updatedPairs);
                                  const newExtraFields = { ...field.value };
                                  delete newExtraFields[pair.key];
                                  if (newKey) {
                                    newExtraFields[newKey] = pair.value;
                                  }
                                  field.onChange(newExtraFields);
                                }}
                                className="flex-1"
                                placeholder="Chave"
                              />
                              <Input
                                ...
                                onChange={(e) => {
                                  ...
                                  const updatedPairs = [...keyValuePairs];
                                  updatedPairs[index] = { ...updatedPairs[index], value: newValue };
                                  setKeyValuePairs(updatedPairs);
                                  field.onChange({
                                    ...field.value,
                                    [pair.key]: newValue,
              ...
              render={({ field }) => (
                <FormItem className="flex flex-col">
                  ...
                  <FormDescription id="direction_id-description">
                    Direção responsável pelo equipamento.
                  </FormDescription>
                  <FormMessage />
                </FormItem>
              )}
