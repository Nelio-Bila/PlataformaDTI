...
//       }
//       else{
//         router.push(`/requests/view-guest?ref=${mutation.data.newRequest.request_number}`);
//       }
//     },
//     onError: (error: Error) => {
...
// Define the expected shape of the newRequest object returned from the API
...
export function RequestForm() {
  ...
  const form = useForm<RequestFormData>({
    ...
    defaultValues: {
      type: undefined,
      requester_name: "",
      items: [{ description: "", quantity: 1, unit: "UM" }],
      requester_direction_id: "",
      requester_department_id: "",
      requester_service_id: "",
      requester_sector_id: "",
  ...
  // Mutation for creating request with typed response
  const mutation = useMutation({
    mutationFn: async (formData: FormData): Promise<NewRequest> => {
      const response = await fetch("/api/requests/create", {
        method: "POST",
        body: formData,
      });
      ...
      return response.json() as Promise<NewRequest>;
    },
    onSuccess: (newRequest: NewRequest) => {
      queryClient.invalidateQueries({ queryKey: ["requests"] });
      form.reset();
      toast({
        title: "Sucesso!",
      ...
      // Redirect based on authentication status using the newRequest data
      if (status === "authenticated") {
        router.push("/requests");
      } else {
        router.push(`/requests/view-guest?ref=${newRequest.request_number}`);
      }
    },
    onError: (error: Error) => {
      form.setError("root", { message: error.message });
        ...
        >
          ...
          <div className="w-full overflow-x-auto">
            <Table className="w-full">
              <TableHeader>
                <TableRow className="min-w-48">
                  <TableHead className="text-xs sm:text-sm">Designação e Característica</TableHead>
                  <TableHead className="text-xs sm:text-sm">Quantidade Solicitada</TableHead>
                  <TableHead className="text-xs sm:text-sm">Acções</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
